...
import { postCategoryRequest } from "../../../../store/config/actions/category/post";
import { putCategoryRequest } from "../../../../store/config/actions/category/put";

const NewCategory = ({ open, onClose, isEdit, toEdit, __, module, maxWidth = "xl", showNoti, setShowNoti, getData, setError }) => {
    const [postCategory, setPostCategory] = useState({ loading: false })
    const [putCategory, setPutCategory] = useState({ loading: false })
    ...
    const onSubmit = (body) => {
        if (isEdit) {
            setPutCategory({ loading: true })
            body.categoryId = get(toEdit, "categoryId")
            putCategoryRequest(body, () => getState)
                .then(({ data }) => {
                    setPutCategory({ loading: false })
                    setShowNoti({ open: true, msg: __(`${module}.msg.update`), variant: "success" })
                    handleClose()
                    getData()
                })
                .catch((err) => { setError(err); setPutCategory({ loading: false }) })
        } else {
            setPostCategory({ loading: true })
            postCategoryRequest(body, () => getState)
                .then(({ data }) => {
                    setPostCategory({ loading: false })
                    setShowNoti({ open: true, msg: __(`${module}.msg.create`), variant: "success" })
                    handleClose()
                    getData()
                })
                .catch((err) => { setError(err); setPostCategory({ loading: false }) })
        }
    };
    ...
    return (
        <>
            <Dialog
                onClose={handleClose}
                aria-labelledby={`modal-${isEdit ? "edit" : "new"}-category-${get(toEdit, "categoryId")}`}
                open={open}
                maxWidth={maxWidth}
                fullWidth
                component="form"
                    ...
                    <Stack className='w-full' direction="row" spacing={2} justifyContent="flex-end">
                        <Stack direction="row" spacing={2}>
                            ...
                            <LoadingButton
                                ...
                                color="primary"
                                type="submit"
                                loading={get(postCategory, "loading") || get(putCategory, "loading")}
                            >
