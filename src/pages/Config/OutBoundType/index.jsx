...
import NewOutBoundType from "./components/NewOutBoundType";
...
import { deleteOutBoundTypeRequest } from "../../../store/config/actions/outBoundType/delete"

const OutBoundType = ({ setBtnFunc }) => {
  ...
  const module = "outBoundType"
  const titles = __(`${module}.table`, { returnObjects: true })

  const outBoundType = useSelector(state => state.config.outBoundType);
  const userState = useSelector(state => state.auth.login.dataUser);
  const getState = useSelector(state => state);

  const getData = ({ page, filterSearch }) => {
    const filters = { page, ...(!!filterSearch && { search: filterSearch }) }
    dispatch(getOutBoundType(filters))
  }
  ...
  const onDelete = () => {
    ...
    setLoadDelete(true)
    deleteOutBoundTypeRequest(body, () => getState)
      .then(({ data }) => {
  ...
  const dataTable = map(get(outBoundType, "data", []), (row, i) => ({
    ...row,
  ...
  return (
    <div>
      <Table headTable={headTable}
        ...
        module={module}
        sizeFilters={125}
        loading={get(outBoundType, "isLoading", false)}
