...
import { postSubCategoryRequest } from "../../../../store/config/actions/subCategory/post";
import { putSubCategoryRequest } from "../../../../store/config/actions/subCategory/put";

const NewSubCategory = ({ open, onClose, isEdit, toEdit, __, module, maxWidth = "xl", showNoti, setShowNoti, getData, setError }) => {
    const [postSubCategory, setPostSubCategory] = useState({ loading: false })
    const [putSubCategory, setPutSubCategory] = useState({ loading: false })
    ...
    const onSubmit = (body) => {
        if (isEdit) {
            setPutSubCategory({ loading: true })
            body.subCategoryId = get(toEdit, "subCategoryId")
            putSubCategoryRequest(body, () => getState)
                .then(({ data }) => {
                    setPutSubCategory({ loading: false })
                    setShowNoti({ open: true, msg: __(`${module}.msg.update`), variant: "success" })
                    handleClose()
                    getData()
                })
                .catch((err) => { setError(err); setPutSubCategory({ loading: false }) })
        } else {
            setPostSubCategory({ loading: true })
            postSubCategoryRequest(body, () => getState)
                .then(({ data }) => {
                    setPostSubCategory({ loading: false })
                    setShowNoti({ open: true, msg: __(`${module}.msg.create`), variant: "success" })
                    handleClose()
                    getData()
                })
                .catch((err) => { setError(err); setPostSubCategory({ loading: false }) })
        }
    };
    ...
    return (
        <>
            <Dialog
                onClose={handleClose}
                aria-labelledby={`modal-${isEdit ? "edit" : "new"}-subCategory-${get(toEdit, "subCategoryId")}`}
                open={open}
                maxWidth={maxWidth}
                fullWidth
                component="form"
                    ...
                    <Stack className='w-full' direction="row" spacing={2} justifyContent="flex-end">
                        <Stack direction="row" spacing={2}>
                            ...
                            <LoadingButton
                                ...
                                color="primary"
                                type="submit"
                                loading={get(postSubCategory, "loading") || get(putSubCategory, "loading")}
                            >
