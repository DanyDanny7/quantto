...
import { deleteSubCategoryRequest } from "../../../store/config/actions/subCategory/delete"

const SubCategory = ({ setBtnFunc }) => {
  ...
  const onDelete = () => {
    ...
    const body = {
      ...
      subCategoryId: get(itemsDelete, "item.subCategoryId"),
    }
    setLoadDelete(true)
    deleteSubCategoryRequest(body, () => getState)
      .then(({ data }) => {
  ...
  const dataTable = map(get(subCategory, "data", []), (row, i) => ({
    ...row,
  ...
  return (
    <div>
      <Table headTable={headTable}
        ...
        module={module}
        sizeFilters={125}
        loading={get(subCategory, "isLoading", false)}
