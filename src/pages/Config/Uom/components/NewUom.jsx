...
import { postUomRequest } from "../../../../store/config/actions/uom/post";
import { putUomRequest } from "../../../../store/config/actions/uom/put";

const NewUom = ({ open, onClose, isEdit, toEdit, __, module, maxWidth = "xl", showNoti, setShowNoti, getData, setError }) => {
    const [postUom, setPostUom] = useState({ loading: false })
    const [putUom, setPutUom] = useState({ loading: false })
    ...
    const onSubmit = (body) => {
        if (isEdit) {
            setPutUom({ loading: true })
            body.uomId = get(toEdit, "uomId")
            putUomRequest(body, () => getState)
                .then(({ data }) => {
                    setPutUom({ loading: false })
                    setShowNoti({ open: true, msg: __(`${module}.msg.update`), variant: "success" })
                    handleClose()
                    getData()
                })
                .catch((err) => { setError(err); setPutUom({ loading: false }) })
        } else {
            setPostUom({ loading: true })
            postUomRequest(body, () => getState)
                .then(({ data }) => {
                    setPostUom({ loading: false })
                    setShowNoti({ open: true, msg: __(`${module}.msg.create`), variant: "success" })
                    handleClose()
                    getData()
                })
                .catch((err) => { setError(err); setPostUom({ loading: false }) })
        }
    };
                    ...
                    <Stack className='w-full' direction="row" spacing={2} justifyContent="flex-end">
                        <Stack direction="row" spacing={2}>
                            ...
                            <LoadingButton
                                ...
                                color="primary"
                                type="submit"
                                loading={get(postUom, "loading") || get(putUom, "loading")}
                            >
