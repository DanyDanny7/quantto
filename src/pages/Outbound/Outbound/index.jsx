...
import { getOutbound } from "../../../store/outbound/thunk/outbound/get"
import { deleteOutboundRequest } from "../../../store/outbound/actions/outbound/delete"
...
const OutBound = () => {
  ...
  const onEdit = () => {
    ...
    // closePoop()
    navegate(`${get(active, "outboundid")}`)
  }
  const onDelete = () => {
    closePoop()
    onDeleteConfirm()
  ...
  const onDeleteElement = () => {
    ...
    const body = {
      ...
      outboundid: get(active, "outboundid")
    }
    setLoadDelete(true)
    deleteOutboundRequest(body, () => getState)
      .then(({ data }) => {
  ...
  const dataTable = map(get(outbound, "data.data", []), (row, i) => ({
    ...row,
  ...
  return (
    ...
    >
      <Table headTable={headTable}
        ...
        module={module}
        sizeFilters={125}
        loading={get(outbound, "isLoading", false)}
      />
      <Popover
        id={"menu-outbound-activo"}
