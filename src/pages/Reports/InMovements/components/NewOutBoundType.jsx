...
import { postOutBoundTypeRequest } from "../../../../store/config/actions/outBoundType/post";
import { putOutBoundTypeRequest } from "../../../../store/config/actions/outBoundType/put";

const NewOutBoundType = ({ open, onClose, isEdit, toEdit, __, module, maxWidth = "xl", showNoti, setShowNoti, getData, setError }) => {
    const [postOutBoundType, setPostOutBoundType] = useState({ loading: false })
    const [putOutBoundType, setPutOutBoundType] = useState({ loading: false })
    ...
    const onSubmit = (body) => {
        if (isEdit) {
            setPutOutBoundType({ loading: true })
            body.type = get(toEdit, "type")
            putOutBoundTypeRequest(body, () => getState)
                .then(({ data }) => {
                    setPutOutBoundType({ loading: false })
                    setShowNoti({ open: true, msg: __(`${module}.msg.update`), variant: "success" })
                    handleClose()
                    getData()
                })
                .catch((err) => { setError(err); setPutOutBoundType({ loading: false }) })
        } else {
            setPostOutBoundType({ loading: true })
            postOutBoundTypeRequest(body, () => getState)
                .then(({ data }) => {
                    setPostOutBoundType({ loading: false })
                    setShowNoti({ open: true, msg: __(`${module}.msg.create`), variant: "success" })
                    handleClose()
                    getData()
                })
                .catch((err) => { setError(err); setPostOutBoundType({ loading: false }) })
        }
    };
    ...
    return (
        <>
            <Dialog
                onClose={handleClose}
                aria-labelledby={`modal-${isEdit ? "edit" : "new"}-outBoundType-${get(toEdit, "type")}`}
                open={open}
                maxWidth={maxWidth}
                fullWidth
                component="form"
                    ...
                    <Stack className='w-full' direction="row" spacing={2} justifyContent="flex-end">
                        <Stack direction="row" spacing={2}>
                            ...
                            <LoadingButton
                                ...
                                color="primary"
                                type="submit"
                                loading={get(postOutBoundType, "loading") || get(putOutBoundType, "loading")}
                            >
...
export default NewOutBoundType