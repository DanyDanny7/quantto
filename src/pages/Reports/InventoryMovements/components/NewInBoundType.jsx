...
import { postInBoundTypeRequest } from "../../../../store/config/actions/inBoundType/post";
import { putInBoundTypeRequest } from "../../../../store/config/actions/inBoundType/put";

const NewInBoundType = ({ open, onClose, isEdit, toEdit, __, module, maxWidth = "xl", showNoti, setShowNoti, getData, setError }) => {
    const [postInBoundType, setPostInBoundType] = useState({ loading: false })
    const [putInBoundType, setPutInBoundType] = useState({ loading: false })
    ...
    const onSubmit = (body) => {
        if (isEdit) {
            setPutInBoundType({ loading: true })
            body.type = get(toEdit, "type")
            putInBoundTypeRequest(body, () => getState)
                .then(({ data }) => {
                    setPutInBoundType({ loading: false })
                    setShowNoti({ open: true, msg: __(`${module}.msg.update`), variant: "success" })
                    handleClose()
                    getData()
                })
                .catch((err) => { setError(err); setPutInBoundType({ loading: false }) })
        } else {
            setPostInBoundType({ loading: true })
            postInBoundTypeRequest(body, () => getState)
                .then(({ data }) => {
                    setPostInBoundType({ loading: false })
                    setShowNoti({ open: true, msg: __(`${module}.msg.create`), variant: "success" })
                    handleClose()
                    getData()
                })
                .catch((err) => { setError(err); setPostInBoundType({ loading: false }) })
        }
    };
    ...
    return (
        <>
            <Dialog
                onClose={handleClose}
                aria-labelledby={`modal-${isEdit ? "edit" : "new"}-inBoundType-${get(toEdit, "type")}`}
                open={open}
                maxWidth={maxWidth}
                fullWidth
                component="form"
                    ...
                    <Stack className='w-full' direction="row" spacing={2} justifyContent="flex-end">
                        <Stack direction="row" spacing={2}>
                            ...
                            <LoadingButton
                                ...
                                color="primary"
                                type="submit"
                                loading={get(postInBoundType, "loading") || get(putInBoundType, "loading")}
                            >
