...
import { getTransfer } from "../../store/transfer/thunk/transfer/get"
import { deleteTransferRequest } from "../../store/transfer/actions/transfer/delete"
...
const Transfer = () => {
  ...
  const onEdit = () => {
    ...
    // closePoop()
    navegate(`${get(active, "transferid")}`)
  }
  const onDelete = () => {
    closePoop()
    onDeleteConfirm()
  ...
  const onDeleteElement = () => {
    ...
    const body = {
      ...
      transferId: get(active, "transferid")
    }
    setLoadDelete(true)
    deleteTransferRequest(body, () => getState)
      .then(({ data }) => {
  ...
  const dataTable = map(get(transfer, "data.data", []), (row, i) => ({
    ...row,
  ...
  return (
    ...
    >
      <Table headTable={headTable}
        ...
        module={module}
        sizeFilters={125}
        loading={get(transfer, "isLoading", false)}
      />
      <Popover
        id={"menu-transfer-activo"}
