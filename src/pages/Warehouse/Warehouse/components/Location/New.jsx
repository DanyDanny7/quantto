...
import { postLocationRequest } from "../../../../../store/warehouse/actions/location/post";
import { putLocationRequest } from "../../../../../store/warehouse/actions/location/put";

const NewLocation = ({ open, onClose, isEdit, toEdit, __, module, maxWidth = "xl", showNoti, setShowNoti, getData, setError }) => {
    const [postLocation, setPostLocation] = useState({ loading: false })
    const [putLocation, setPutLocation] = useState({ loading: false })
    const { id } = useParams();
    ...
    const onSubmit = (values) => {
        ...
        if (isEdit) {
            setPutLocation({ loading: true })
            body.locationId = get(toEdit, "locationId")
            putLocationRequest(body, () => getState)
                .then(({ data }) => {
                    setPutLocation({ loading: false })
                    setShowNoti({ open: true, msg: __(`${module}.msg.update`), variant: "success" })
                    handleClose()
                    getData()
                })
                .catch((err) => { setError(err); setPutLocation({ loading: false }) })
        } else {
            setPostLocation({ loading: true })
            postLocationRequest(body, () => getState)
                .then(({ data }) => {
                    setPostLocation({ loading: false })
                    setShowNoti({ open: true, msg: __(`${module}.msg.create`), variant: "success" })
                    handleClose()
                    getData()
                })
                .catch((err) => { setError(err); setPostLocation({ loading: false }) })
        }
    };
    ...
    return (
        <>
            <Dialog
                onClose={handleClose}
                aria-labelledby={`modal-${isEdit ? "edit" : "new"}-location-${get(toEdit, "locationId")}`}
                open={open}
                maxWidth={maxWidth}
                fullWidth
                component="form"
                    ...
                    <Stack className='w-full' direction="row" spacing={2} justifyContent="flex-end">
                        <Stack direction="row" spacing={2}>
                            ...
                            <LoadingButton
                                ...
                                color="primary"
                                type="submit"
                                loading={get(postLocation, "loading") || get(putLocation, "loading")}
                            >
